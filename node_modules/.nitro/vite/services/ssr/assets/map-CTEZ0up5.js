...
import { ChevronRight, LocateFixed, Minus, Plus, Search, X } from "lucide-react";
...
].map(({ iso3, isoNumeric, name, region }) => ({
	iso3,
	isoNumeric,
	name,
	region
}));
var CATALOG_BY_ISO3 = new Map(COUNTRIES.map((r) => [r.iso3, r]));
var CATALOG_BY_NUMERIC = new Map(COUNTRIES.map((r) => [r.isoNumeric, r]));
function mergePulses(pulses) {
	const extra = new Map(pulses.map((p) => [p.iso3, p]));
	const used = /* @__PURE__ */ new Set();
	const merged = COUNTRIES.map((row) => {
		used.add(row.iso3);
		return {
			...row,
			pulses: extra.get(row.iso3)?.pulses ?? 0
		};
	});
...
function rankOf(rows, iso3) {
	return [...rows].sort((a, b) => b.pulses - a.pulses).findIndex((r) => r.iso3 === iso3) + 1;
}
function toPulseRows(pulses) {
	return (Array.isArray(pulses) ? pulses.map((p) => [p.iso3, p.pulses]) : Object.entries(pulses)).map(([iso3, n]) => ({
		iso3,
...
var LAND_EMPTY = "#1c232c";
var compact = new Intl.NumberFormat("en", {
	notation: "compact",
	maximumFractionDigits: 1
});
var integer = new Intl.NumberFormat("en", { maximumFractionDigits: 0 });
function formatPulses(value) {
	if (!Number.isFinite(value)) return "—";
	return value >= 1e4 ? compact.format(value) : integer.format(value);
}
...
function makeColorScale(values) {
	...
	const scale = scaleThreshold().domain(unique).range([...CHLORO_STOPS]);
	return {
		color: (value) => {
			if (value === void 0 || !Number.isFinite(value) || value <= 0) return LAND_EMPTY;
			return scale(value);
		},
		breaks: unique,
		stops: CHLORO_STOPS
...
//#endregion
//#region src/components/map/world-choropleth.tsx
var topology = countries_110m_default;
var collection = feature(topology, topology.objects.countries);
function isoNumeric(id) {
	return String(id ?? "").padStart(3, "0");
}
var WorldChoropleth = forwardRef(function WorldChoropleth({ rows, selectedId, originId, onSelect, className }, ref) {
	...
	const projection = useMemo(() => {
		...
		return geoNaturalEarth1().fitExtent([[16, 24], [size.w - 16, size.h - 12]], { type: "Sphere" });
	}, [size]);
	...
	useEffect(() => {
		...
		const behavior = zoom().scaleExtent([1, 8]).clickDistance(6).extent([[0, 0], [size.w, size.h]]).translateExtent([[-size.w * .35, -size.h * .35], [size.w * 1.35, size.h * 1.35]]).on("zoom", (event) => {
	...
	useImperativeHandle(ref, () => ({
		...
		focusCountry: (numeric) => {
			...
			const k = Math.max(1.2, Math.min(6, .55 / Math.max(dx / size.w, dy / size.h)));
			const t = zoomIdentity.translate(size.w / 2, size.h / 2).scale(k).translate(-cx, -cy);
			z.transform(select(svg), t);
		}
	}), [pathGen, size]);
	const isClick = useCallback((clientX, clientY) => {
	...
	return /* @__PURE__ */ jsxs("div", {
		ref: wrapRef,
		className: cn("relative h-full w-full overflow-hidden bg-ocean", className),
		children: [size.w > 0 && pathGen ? /* @__PURE__ */ jsxs("svg", {
			ref: svgRef,
			width: size.w,
			height: size.h,
			...
			children: [/* @__PURE__ */ jsx("rect", {
				...
				children: [
					...
					originId ? /* @__PURE__ */ jsx(OriginPulse, {
						pathGen,
						originId
					}) : null
				]
			})]
		}) : /* @__PURE__ */ jsx("div", { className: "h-full w-full bg-ocean" }), hover ? /* @__PURE__ */ jsx(MapTooltip, { tip: hover }) : null]
	});
});
var CountryLayer = memo(function CountryLayer({ pathGen, lands, selectedId, isClick, onHover, onSelect }) {
	return collection.features.map((feat, index) => {
		...
		if (!info) return null;
		return /* @__PURE__ */ jsx(CountryPath, {
			id,
			d,
			info,
...
var CountryPath = memo(function CountryPath({ id, d, info, selected, isClick, onHover, onSelect }) {
...
function OriginPulse({ pathGen, originId }) {
	...
	return /* @__PURE__ */ jsxs("g", {
		...
		children: [/* @__PURE__ */ jsx("circle", {
			...
			className: "origin-pulse-dot",
			r: "2.8"
		})]
	});
}
function MapTooltip({ tip }) {
	const pad = 12;
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsx("p", {
			...
			children: tip.value === void 0 || tip.value <= 0 ? "No pulses yet" : `${formatPulses(tip.value)} ${tip.value === 1 ? "pulse" : "pulses"}`
		})]
	});
}
//#endregion
//#region src/components/map/origin-dag.tsx
function OriginDag({ rows, selected }) {
	...
	return /* @__PURE__ */ jsxs("div", { children: [/* @__PURE__ */ jsx("p", {
		className: "mb-2 font-mono text-xs tracking-wide text-subtle uppercase",
		children: "Provenance DAG"
	}), /* @__PURE__ */ jsx("ol", {
		className: "flex list-none items-stretch gap-0 p-0",
		"aria-label": "Origin provenance",
		children: nodes.map((node, i) => /* @__PURE__ */ jsxs("li", {
			className: "flex min-w-0 flex-1 items-stretch",
			children: [i > 0 ? /* @__PURE__ */ jsx("span", {
				className: "flex w-4 shrink-0 items-center justify-center text-subtle",
				"aria-hidden": true,
				children: /* @__PURE__ */ jsx(ChevronRight, { className: "size-3.5" })
			}) : null, /* @__PURE__ */ jsxs("div", {
				className: cn("min-w-0 flex-1 rounded-sm px-2 py-1.5", node.active ? "bg-surface-2" : "bg-surface"),
				children: [/* @__PURE__ */ jsx("p", {
					className: "truncate text-xs text-subtle",
					children: node.label
				}), /* @__PURE__ */ jsx("p", {
					className: "truncate font-mono text-xs tabular-nums text-fg",
					children: formatPulses(node.value)
				})]
			})]
		}, node.key))
	})] });
...
//#endregion
//#region src/components/map/region-bars.tsx
var ORDER = [
	"Europe",
	"Americas",
	"Asia-Pacific",
...
function RegionBars({ rows }) {
	...
	return /* @__PURE__ */ jsx("ul", {
		...
		children: grouped.map((g) => /* @__PURE__ */ jsxs("li", { children: [/* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsx("span", {
				...
				children: formatPulses(g.value)
			})]
		}), /* @__PURE__ */ jsx("span", {
			className: "mt-1 block h-1 overflow-hidden rounded-full bg-land",
			children: /* @__PURE__ */ jsx("span", {
...
//#endregion
//#region src/components/map/country-panel.tsx
function CountryPanel({ rows, selected, onClear, className }) {
	if (!selected) return /* @__PURE__ */ jsx(GlobalSnapshot, {
		rows,
		className
	});
		...
		children: [
			/* @__PURE__ */ jsxs("header", {
				...
				children: [/* @__PURE__ */ jsxs("div", {
					...
					children: [
						...
						/* @__PURE__ */ jsx("h2", {
							className: "font-display text-2xl leading-tight tracking-tight text-fg lg:text-3xl",
							children: selected.name
						}),
						/* @__PURE__ */ jsx("p", {
							className: "mt-1 font-mono text-xs text-muted",
							children: selected.iso3
						})
					]
				}), /* @__PURE__ */ jsx(Button, {
					variant: "ghost",
			...
			/* @__PURE__ */ jsxs("div", { children: [
				/* @__PURE__ */ jsx("p", {
					...
					children: "Origin pulses"
				}),
				/* @__PURE__ */ jsx("p", {
					className: "mt-1 font-display text-4xl leading-none tabular-nums tracking-tight text-fg",
					children: formatPulses(selected.pulses)
				}),
				/* @__PURE__ */ jsx("p", {
					className: "mt-2 text-sm text-muted",
					children: selected.pulses > 0 ? `Rank ${rank} of ${pulsed.length} · ${share} of recorded pulses` : "No pulses from this origin yet"
				})
			] }),
			/* @__PURE__ */ jsx(OriginDag, {
				rows,
				selected
			}),
			/* @__PURE__ */ jsx("p", {
				className: "text-xs leading-relaxed text-muted",
				children: selected.pulses > 0 ? `${selected.pulses} live ${selected.pulses === 1 ? "pulse" : "pulses"} recorded from ${selected.name}. Each Record origin adds one.` : `Nobody has pulsed ${selected.name} yet. Record origin if that's you.`
			})
		]
	});
}
function GlobalSnapshot({ rows, className }) {
	const pulsed = rows.filter((r) => r.pulses > 0);
	const total = totals(pulsed);
		...
		children: [
			/* @__PURE__ */ jsxs("header", { children: [
				...
				/* @__PURE__ */ jsx("p", {
					...
					children: "Pulses from recorded visits, grouped by inferred country."
				})
			] }),
			...
			/* @__PURE__ */ jsxs("dl", {
				...
				children: [
					...
					/* @__PURE__ */ jsx(Stat, {
						label: "Median",
						value: formatPulses(medianOf(pulsed))
					}),
					/* @__PURE__ */ jsx(Stat, {
						label: "Catalog",
			...
			/* @__PURE__ */ jsx(RegionBars, { rows: pulsed }),
			/* @__PURE__ */ jsx("p", {
				className: "text-xs text-subtle",
				children: "Click a country on the map, or pick one from the list."
			})
		]
	});
}
function Stat({ label, value }) {
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsx("dt", {
			...
			className: "mt-0.5 truncate text-sm font-medium text-fg",
			children: value
		})]
	});
}
...
//#endregion
//#region src/components/map/ranking.tsx
function Ranking({ rows, selectedId, query, onQuery, onSelect }) {
	const pulsed = rows.filter((r) => r.pulses > 0);
	const total = totals(pulsed);
	const q = query.trim().toLowerCase();
	const sorted = [...pulsed].sort((a, b) => b.pulses - a.pulses);
	const filtered = q ? rows.filter((r) => r.name.toLowerCase().includes(q) || r.iso3.toLowerCase().includes(q) || r.region.toLowerCase().includes(q)).sort((a, b) => b.pulses - a.pulses) : sorted.slice(0, 16);
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [/* @__PURE__ */ jsxs("div", {
			...
			children: [/* @__PURE__ */ jsx("p", {
				...
				children: q ? "Matches" : "Top origins"
			}), /* @__PURE__ */ jsxs("label", {
				className: "relative block",
				children: [/* @__PURE__ */ jsx(Search, { className: "pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2 text-subtle" }), /* @__PURE__ */ jsx("input", {
					type: "text",
					value: query,
			...
			children: filtered.length === 0 ? /* @__PURE__ */ jsx("li", {
				...
				children: q ? "No countries match." : "No origin pulses yet. Record yours from the wallet."
			}) : filtered.map((row, i) => {
				...
				return /* @__PURE__ */ jsx("li", { children: /* @__PURE__ */ jsxs("button", {
					...
					className: cn("flex min-h-11 w-full items-center gap-3 rounded-md px-3 py-2 text-left transition-colors duration-150", active ? "bg-surface-2" : "hover:bg-surface-2/70"),
					children: [
						/* @__PURE__ */ jsx("span", {
							className: "w-5 shrink-0 font-mono text-xs tabular-nums text-subtle",
							children: q ? sorted.findIndex((r) => r.iso3 === row.iso3) + 1 || "—" : i + 1
						}),
						/* @__PURE__ */ jsxs("span", {
							...
							children: [/* @__PURE__ */ jsxs("span", {
								...
								children: [/* @__PURE__ */ jsx("span", {
									...
									children: formatPulses(row.pulses)
								})]
							}), /* @__PURE__ */ jsx("span", {
								className: "mt-1 block h-1 overflow-hidden rounded-full bg-land",
								children: /* @__PURE__ */ jsx("span", {
						...
						/* @__PURE__ */ jsx("span", {
							className: "hidden w-12 shrink-0 text-right font-mono text-xs tabular-nums text-subtle sm:block",
							children: formatShare(row.pulses, total)
						})
					]
				}) }, row.iso3);
			})
...
//#endregion
//#region src/components/map/metric-switcher.tsx
function OriginsChip({ className }) {
	return /* @__PURE__ */ jsxs("div", {
		className: cn("rounded-lg bg-surface/90 px-3 py-2 shadow-border backdrop-blur-sm", className),
		children: [/* @__PURE__ */ jsx("p", {
			className: "text-xs font-medium tracking-wide text-subtle uppercase",
			children: "Origin pulses"
		}), /* @__PURE__ */ jsx("p", {
			className: "mt-0.5 text-sm text-fg",
			children: "Where recorded visits land"
		})]
	});
}
//#endregion
//#region src/components/map/legend.tsx
function ChoroplethLegend({ scale, className }) {
	const { breaks, stops } = scale;
	const labels = [formatPulses(0), ...breaks.map((b) => formatPulses(b))];
	return /* @__PURE__ */ jsxs("div", {
		className: cn("rounded-lg bg-surface/90 px-3 py-2.5 shadow-border backdrop-blur-sm", className),
		children: [
			/* @__PURE__ */ jsx("p", {
				className: "mb-1.5 text-xs font-medium tracking-wide text-subtle uppercase",
				children: "Pulses"
			}),
			/* @__PURE__ */ jsx("div", {
				className: "flex h-2 overflow-hidden rounded-full",
				children: stops.map((c) => /* @__PURE__ */ jsx("span", {
					className: "h-full flex-1",
					style: { background: c }
				}, c))
			}),
			/* @__PURE__ */ jsxs("div", {
				className: "mt-1.5 flex justify-between gap-2 font-mono text-xs tabular-nums text-muted",
				children: [
					/* @__PURE__ */ jsx("span", { children: labels[0] }),
					/* @__PURE__ */ jsx("span", { children: labels[Math.floor(labels.length / 2)] }),
					/* @__PURE__ */ jsx("span", { children: labels[labels.length - 1] === labels[0] ? "High" : labels[labels.length - 1] })
				]
			})
		]
	});
...
//#endregion
//#region src/components/map/zoom-controls.tsx
function ZoomControls({ onZoomIn, onZoomOut, onReset, className }) {
	return /* @__PURE__ */ jsxs("div", {
		className: cn("flex flex-col overflow-hidden rounded-lg bg-surface/90 shadow-border backdrop-blur-sm", className),
		children: [
			/* @__PURE__ */ jsx(Button, {
				variant: "ghost",
				size: "icon",
				className: "size-10 rounded-none text-fg",
				onClick: onZoomIn,
				"aria-label": "Zoom in",
...
//#endregion
//#region src/lib/geo/infer-origin.ts
var TZ_TO_ISO3 = {
	"Europe/Zagreb": "HRV",
	"Europe/Belgrade": "SRB",
...
function fromIso3(iso3) {
	if (!iso3) return null;
	const row = CATALOG_BY_ISO3.get(iso3);
	if (row) return {
		iso3: row.iso3,
		name: row.name
	};
...
//#endregion
//#region src/components/map/dashboard.tsx
function OriginsDashboard() {
	...
	const pulses = useMemo(() => {
		...
		for (const p of local) byIso.set(p.iso3, Math.max(byIso.get(p.iso3) ?? 0, p.pulses));
		return toPulseRows([...byIso.entries()].map(([iso3, n]) => ({
			iso3,
			pulses: n
		})));
	}, [originPulses, remote]);
	...
	const recordOrigin = useCallback(() => {
		...
		pulseOrigin(o.iso3);
		api(`/api/origin?iso3=${o.iso3}`, "POST").then(() => loadOrigins()).catch(() => void 0);
	...
	const selectFromList = (isoNumeric) => {
		setSelectedId(isoNumeric);
		mapRef.current?.focusCountry(isoNumeric);
	};
	return /* @__PURE__ */ jsxs("div", {
		className: "flex min-h-0 flex-1 flex-col overflow-hidden bg-bg lg:flex-row",
		children: [/* @__PURE__ */ jsxs("div", {
			className: "relative min-h-[46vh] min-w-0 flex-1 lg:min-h-0",
			children: [/* @__PURE__ */ jsx(WorldChoropleth, {
				ref: mapRef,
				rows,
				selectedId,
				originId,
				...
				className: "pointer-events-none absolute inset-0 z-10 p-3 md:p-4",
				children: [
					/* @__PURE__ */ jsxs("div", {
						className: "flex items-start justify-between gap-3",
						children: [/* @__PURE__ */ jsx("div", {
							className: "pointer-events-auto min-w-0",
							children: /* @__PURE__ */ jsx(OriginsChip, {})
						}), /* @__PURE__ */ jsx("div", {
					...
					/* @__PURE__ */ jsx("div", {
						...
						children: /* @__PURE__ */ jsx(ChoroplethLegend, {
							scale,
							className: "pointer-events-auto"
						})
					}),
					/* @__PURE__ */ jsx("div", {
						className: "absolute right-3 bottom-3 hidden max-w-56 md:block lg:right-4 lg:bottom-4",
						children: /* @__PURE__ */ jsxs("div", {
							className: "pointer-events-auto rounded-lg bg-surface/90 px-3 py-2 shadow-border backdrop-blur-sm",
							children: [/* @__PURE__ */ jsx("p", {
								className: "text-xs leading-relaxed text-muted",
								children: origin ? /* @__PURE__ */ jsxs(Fragment, { children: ["Visiting from ", /* @__PURE__ */ jsx("span", {
									className: "text-fg",
			...
			className: "flex max-h-[48vh] min-h-0 w-full shrink-0 flex-col overflow-y-auto border-t border-border bg-bg lg:max-h-none lg:w-80 lg:overflow-hidden lg:border-t-0 lg:border-l xl:w-96",
			children: [
				/* @__PURE__ */ jsx("div", {
					className: "px-5 pt-4 md:hidden",
					children: /* @__PURE__ */ jsx(Button, {
						type: "button",
						className: "h-11 w-full",
						onClick: recordOrigin,
						children: "Record my origin"
					})
				}),
				/* @__PURE__ */ jsx(CountryPanel, {
					rows,
					selected,
					onClear: () => setSelectedId(null),
					className: "shrink-0"
				}),
				/* @__PURE__ */ jsx(Ranking, {
					rows,
					selectedId,
					query,
					onQuery: setQuery,
...
function MapPage() {
	return /* @__PURE__ */ jsx(Shell, { children: /* @__PURE__ */ jsx(OriginsDashboard, {}) });
}
//#endregion
